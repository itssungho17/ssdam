# Example: User Input Prompt for user-input.template

> **How to use this file:**
> Copy the entire content below (from `<protocol>` to `</output_format>`)
> and paste it into any AI chat tool (Claude, ChatGPT, Gemini, etc.).
> The `<user_input>` section at the bottom contains the user's actual idea.
> The AI should produce a SINGLE YAML document as output — no markdown, no prose.
...
<user_input>
idea_raw: |
  Spring Boot, PostgreSQL, Svelte 학습 목적으로 토이 프로젝트를 하려고 하는데,
  기능부터 정리하려고. 아래는 내가 대강 생각하는 기능과 구현이야.

  [ 주요 기능 ]
  1. 기본으로 제공되는 이미지/동영상 파일 조회 기능(3Depth)
  2. 사용자가 파일 업로드 기능(이미지/동영상 파일만 가능함)
  3. 타 사용자가 업로드 한 파일 조회 기능
  4. 타 사용자가 업로드 한 파일 구매 기능
  5. 구매한 파일은 4번에 표시 및 1번과 함께 확인 가능

  [ 페이지 ]
  1. 소유한 모든 자료를 모아보는 페이지
  1-1) 3depth로 구성되며, 폴더, 하위폴더, 자료 순으로 구성됨.
  1-2) 내가 보유한 모든 이미지/동영상 표시(숨기기 자료 제외)
  1-3) 더블클릭 등의 방법으로 자세히 보기 기능 지원(동영상의 경우 재생까지)
  1-4) 편집 모드 선택 시, 각 이미지/동영상의 이름변경/이동/복사/숨기기 가능
  2. 내가 업로드 한 파일 확인 및 업로드 페이지
  2-1) 업로드 한 파일 전체 조회 (폴더 구분 X, 업로드 일자 별 구분)
  2-2) 신규 파일 업로드
  2-3) 업로드 한 파일의 이름 변경/타 사용자에게 표시 여부 결정
  2-4) 타 사용자가 구매한 횟수 표시
  3. 다른 사용자가 업로드 한 파일 페이지
  3-1) 다른 사용자가 업로드 한 파일 조회
  3-2) 이름 및 #태그 로 검색 기능
  3-3) 업로드 한 파일 구매 기능
  3-4) 구매한 파일에 테두리, 체크 표시 등 구분 기능(혹은 구매한 자료 숨기기 기능)
constraints: |
  - 학습 목적 토이 프로젝트
  - 기술: Spring Boot + PostgreSQL + Svelte
