...
|---|---|---|
| Stage Spec | `project_id`, `stage_id`, `requirement_ids` | `policy_id`, `checkpoint_id` |
| Execution | `execution_id`, `stage_id` | Input contract basis, generated `artifact_id` |
| Artifact | `artifact_id`, `stage_id` | `requirement_ids`, location/version/hash |
...
| Checkpoint | `checkpoint_id`, `stage_id` | `evaluation_id`, `evidence_id`, `policy_id` |
| Recovery | `recovery_id`, `stage_id` | source checkpoint, change target, re-evaluation result |
...
- Did repeated FAIL frequency of the same Stage exceed policy threshold?
