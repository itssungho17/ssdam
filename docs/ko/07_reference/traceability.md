...
| Execution | `execution_id`, `stage_id` | 입력 계약 근거, 생성 `artifact_id` |
| Artifact | `artifact_id`, `stage_id` | `requirement_ids`, 위치/버전/해시 |
...
| Recovery | `recovery_id`, `stage_id` | source checkpoint, 변경 대상, 재평가 결과 |
...
- 동일 스테이지 반복 FAIL 횟수가 정책 임계값을 초과했는가?
